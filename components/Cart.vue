          ...
          li.flex.py-6(v-for='(product, index) in cart' :key='index')
            ...
            .ml-4.flex.flex-1.flex-col
              ...
              .flex.flex-1.items-end.justify-between.text-sm
                div.flex
                  //input.form-select.form-select-lg.block.px-2.py-1.text-gray-700.bg-white.border-2.border-solid.border-gray-800.rounded.h-9(:value="product.quantity" disabled aria-label='Anzahl auswählen')
                  //input.ml-3.form-select.form-select-lg.mr-2.block.px-2.py-1.text-gray-700.bg-white.border-2.border-solid.border-gray-800.rounded.h-9(:value="unit(product.unit, product.units)" disabled aria-label='Variante auswählen')
                .flex
...
export default {
  ...
  computed: {
    unit () {
      return (product) => {
         return product.units.find(unit => unit.value == product.unit).name;
      }
    },
    cartCount () {
